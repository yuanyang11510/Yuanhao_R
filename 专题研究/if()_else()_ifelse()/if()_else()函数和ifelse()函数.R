...
#  条件中的n()的结果是一个标量（来自分组，每个分组共享一个n()），长度为1，而分支中的paste(C,"=1")和paste(C,">1")的结果是向量，长度为每个分组中的行数，条件向量的长度和结果向量的长度不一致，if_else()函数会直接报错。
...
x2
# 使用ifelse()函数的做法虽然表面上达到了正确的结果，但底层逻辑和if()...else()结构不同，由于条件向量的长度为1，ifelse()函数实际在每个分组中只会得到第一行的结果，然后将该结果自动扩展到分组中的其他行，只是此处分组中各行的内容恰好相同，因此结果恰好是正确的，如果需要处理的数据在同一分组内的每一行的内容并不总是相同，ifelse()函数的做法就会导致难以察觉的错误。
...
x3
# 使用if()...else()结构的做法是先在以分组为单位先做一次条件判断，然后对分组中的每一行应用同一个分支函数，因此结果才是真正向量化的，如果需要处理的数据在同一分组内的每一行的内容并不总是相同，if()...else()结构的做法才能得到正确结果。
